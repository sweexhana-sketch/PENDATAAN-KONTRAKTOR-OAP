...
export default function NeonAdapter(client: pg.Pool): NeonAdapter {
  return {
    ...
    async getUserByEmail(email) {
      console.log('ADAPTER: getUserByEmail start for:', email);
      const sql = 'select * from auth_users where email = $1';
      console.log('ADAPTER: running query 1...');
      const result = await client.query(sql, [email]);
      console.log('ADAPTER: query 1 done, rowCount:', result.rowCount);
      if (result.rowCount === 0) {
        return null;
      }
      const userData = result.rows[0];
      console.log('ADAPTER: running query 2 for userId:', userData.id);
      const accountsData = await client.query(
        'select * from auth_accounts where "userId" = $1',
        [userData.id]
      );
      console.log('ADAPTER: query 2 done, accounts count:', accountsData.rowCount);
      return {
        ...userData,
